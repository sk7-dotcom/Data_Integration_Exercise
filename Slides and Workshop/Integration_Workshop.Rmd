...
peaks_closest_features <- read.delim("MACS3/peaks_features_subset.txt", header = FALSE)
...
colnames(peaks_features_split) <- c("geneid", "name", "distance")

peaks_feature_trim <- select(peaks_features_split, geneid,  distance)
...
dedup_features <- unique(setDT(cleaned_features)[order(geneid, -distance)], by = "geneid")

<<<<<<< HEAD
=======
<<<<<<< HEAD
write.csv(dedup_features, file = '~/Desktop/ChIP_table.csv')

ChIP_dat <- read.csv('ChIP/ChIP_table.csv')
ChIP_dat <- ChIP_dat %>% mutate(geneid = as.numeric(geneid)) %>% drop_na(geneid) 
=======
ChIP_dat <- dedup_closest %>% mutate(geneid = as.numeric(geneid)) %>% drop_na() 
>>>>>>> 8901657b001eb395aa76b067d321f90a257497b2

>>>>>>> c91f79f04b753ae155a98248fc2b1cae67c91028
```
